...
def make_git_commit_push_script():
    return _LOG_HEAD.format(tag="git-commit-push") + r'''emulate -L zsh
...
def make_copy_path_script():
    return _LOG_HEAD.format(tag="copy-path") + r'''if [ "$#" -eq 0 ]; then
...
def service_defs(docx_path):
    ...
    return [
        ...
        ("新建文本文件",       "new_txt.sh",      make_shell_script("txt",  "未命名"),                       "", False),
        ...
        ("提交并推送当前仓库", "git_commit_push.sh", make_git_commit_push_script(),                           "", False),
